PartKeeprMobile.ServerView = Ext.extend(Ext.Panel, {
	...
	addServer: function () {
		var j = Ext.ModelMgr.create({}, 'PartKeeprMobile.ServerListModel');
	...
	onServerSave: function (editor) {
		...
		editor.record.checkConnectivity(Ext.createDelegate(this.saveServer, this));
	},
	saveServer: function (record) {
		if (record.phantom) {
			Ext.data.Model.id(record);
			this.serverList.store.add(record);
		} else {
			record.save();
		}
